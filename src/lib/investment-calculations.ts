 ...
 */
export function calculateInvestmentMetrics(data: InvestmentData): CalculationResults {
  ...
  const effectiveAnnualRent = annualNetRentAfterCharges * (1 - rentalIncome.vacancy / 100) * (1 - rentalIncome.management / 100);
  ...
  if (!financing.isCashPurchase && financing.loanAmount > 0) {
    ...
    monthlyPayment = financing.loanAmount * 
  ...
  const resalePrice = totalProjectCost * Math.pow(1 + projection.propertyAppreciation / 100, projection.holdingPeriod);
  ...
  if (!financing.isCashPurchase && financing.loanAmount > 0) {
    ...
    if (paymentsMade < totalPayments) {
      remainingDebt = financing.loanAmount * 
  ...
  // Enrichissement net
  const netEnrichment = resalePrice - remainingDebt + availableTreasury - financing.personalContribution;
  const enrichmentMultiplier = financing.personalContribution > 0 ? netEnrichment / financing.personalContribution : 0;
...
export function calculateNetYield(
  ...
  const totalAnnualCharges = taxeFonciere + chargesAnnuelles + assuranceProprietaire;
  const annualNetRent = annualGrossRent - totalAnnualCharges;
  const effectiveAnnualRent = annualNetRent * (1 - vacancy / 100) * (1 - management / 100);
...
export function calculateMonthlyCashFlow(
  ...
  const monthlyCharges = (taxeFonciere + chargesAnnuelles + assuranceProprietaire) / 12;
  const effectiveMonthlyRent = monthlyRent * (1 - vacancy / 100) * (1 - management / 100);
